...
async function handleBookmarkRequest(req, res) {
    let bookmarkRepository = new Repository("./bookmarks.json");//Modifier pour oter contacts.json
    let bookmark = null;
    if (req.url == "/api/bookmarks") { //Modifier pour oter contacts
        switch (req.method) {
            case "GET":
        ...
        if (req.url.includes("/api/bookmarks/")) { //Modifier pour oter contacts#######################
            let id = parseInt(req.url.substring(req.url.lastIndexOf("/") + 1, req.url.length));
            switch (req.method) {
                case "GET":
                    let contact = bookmarkRepository.get(id);
                    if (contact !== null)
...
const server = createServer(async (req, res) => {
    ...
    if (!CORS_Preflight(req, res))
        if (!handleBookmarkRequest(req, res))
            response(res, 404);
});
